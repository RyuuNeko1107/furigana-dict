...
|---|---|---|---|---|---|---|
| 2026-05-13 (R25 後 fresh) | v2026.05.13.7 (= round 25 batch 1-26、 +1621 件 一般語彙) — **fresh seed 55555** (= 過去未使用) | 9,192 / 10,000 | _(未算定)_ | _(同 baseline ≈ 95%)_ | _(未算定)_ | **87.0%** |
...
>
> **\*ja 正答率の数値算定について** (= 2026-05-13 修正):
> - VOICEVOX engine は外部 baseline で dict 変更不変 (= 同 corpus / 同 seed / 同 engine version) のため、
>   旧 round 19-20 review で確定した **95.0% を継続採用**。
> - 両者一致 84.1% → 87.1% (+3.0pt) の上昇は、 旧 diff 内の 「vv correct, ja wrong」 だった ~275 件が
>   ja 改善で 「両者一致」 に吸収された結果と数学的に説明可能 (= VV total correct = 7720+1000 → 7995+725 = 同 8720)。
> - これより ja 正答率 = 87.1% + (= 旧 ja-only-in-diff + 移動分) → **97.9% 想定**、 ja − VV = **+2.9pt**。
> - 初回 sample n=200 Agent classify は vv-correct-in-diff を ~22% under-count しており信頼性低、
>   正式値は次 round で full 1184 件 ground truth review 後に確定する。
>
> 他 seed (verify/33333/55555) は VV 同 baseline を仮定 + 両者一致 ratio から同様に推定可能、
> 数値は次 review round で確定する (= 現状は両者一致のみ確定数値)。
